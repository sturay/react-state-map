...
export function activate(context: vscode.ExtensionContext) {
        ...
        async (uri?: vscode.Uri) => {
            try {
                ...
                vscode.window.showInformationMessage(
                    `Found ${analysisResult.components.length} React components`
                );
    ...
    const analyzeCurrentCommand = vscode.commands.registerCommand(
        'reactVisualizer.analyzeCurrent',
        async () => {
            ...
            if (!/\.(js|jsx|ts|tsx)$/.test(filePath)) {
                vscode.window.showErrorMessage('Please open a React file');
                return;
            }

            vscode.commands.executeCommand('reactVisualizer.showDiagram', activeEditor.document.uri);
        }
    );

    // Command to analyze entire workspace
    const analyzeWorkspaceCommand = vscode.commands.registerCommand(
        'reactVisualizer.analyzeWorkspace',
        () => {
            vscode.commands.executeCommand('reactVisualizer.showDiagram');
        }
    );

    // Register file watcher for auto-refresh
    const config = vscode.workspace.getConfiguration('reactVisualizer');
    ...
    const hasReactFiles = async () => {
        const reactFiles = await vscode.workspace.findFiles('**/*.{jsx,tsx}', '**/node_modules/**', 1);
        await vscode.commands.executeCommand('setContext', 'workspaceHasReactFiles', reactFiles.length > 0);
    };
